...
final class _Harness {
  ...
  late final settings = SettingsController(settingsRepository: settingsRepository);
...
void main() {
  ...
  test('до загрузки настроек список ждёт, потом читается папка загрузок и за ней следят', () async {
  ...
  test('смена папки в настройках показывает её видео и следит уже за ней', () async {
    final harness = _Harness();

    harness.repository.folders[_videos] = [testLibraryVideo('c', folder: _videos)];
  ...
  test('превью и длительность догружаются по одному сверху вниз; сбой не повторяется', () async {
    final harness = _Harness();
  ...
  test('позиция просмотра сохраняется и видна сразу; длительность плеера точнее', () async {
    final harness = _Harness();
    ...
    expect(harness.repository.savedPositions.single.position, const Duration(minutes: 2));

    /// A video no longer in the folder is not saved
    await harness.controller.savePosition('gone', position: const Duration(seconds: 5));
  ...
  test('чтение папки, начатое до сохранения позиции, не откатывает её на экране', () async {
    final harness = _Harness();

    await harness.start();

    await harness.controller.savePosition('a', position: const Duration(minutes: 4));

    /// The reading got the database before the position was written
    harness.repository.folders[_downloads] = [testLibraryVideo('a'), testLibraryVideo('b')];
  ...
  test('пропавшая папка: ошибка с путём и пустой список; повтор после появления папки', () async {
    final harness = _Harness();
    ...
    expect(harness.state.failure?.code, const PlayerErrorCodes().folderNotFound);
