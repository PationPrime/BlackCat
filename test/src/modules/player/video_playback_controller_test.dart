...
final class _Harness {
  ...
  late final library = VideoLibraryController(
  ...
  List<Duration> get saved => [
    for (final video in repository.savedPositions) video.position,
  ];
  ...
  void moveTo(Duration position, {bool? isPlaying, bool? isCompleted}) =>
      player.emit(
...
void main() {
  ...
  test(
    'видео открывается с места остановки, недосмотренное совсем чуть-чуть и досмотренное — с начала',
    () async {
  ...
  test(
    'позиция сохраняется при паузе, раз в интервал, в конце и при закрытии',
    () async {
      final harness = _Harness();

      await harness.open(
        testLibraryVideo('a'),
        positionSaveInterval: const Duration(milliseconds: 200),
      );
  ...
  test('закрытие до ответа плеера не затирает сохранённую позицию', () async {
    ...
    await harness.open(
      testLibraryVideo('a', position: const Duration(minutes: 3)),
    );
    await harness.controller.close();
  ...
  test(
    'ошибка открытия показывается, «Повторить» открывает видео заново',
    () async {
      final harness = _Harness();
      ...
      expect(
        ...
        contains('Не удалось воспроизвести видео'),
      );
      expect(harness.state.isOpening, isFalse);
      ...
      expect(
        harness.player.calls.where((call) => call == 'open'),
        hasLength(2),
      );

      /// A player error during playback is shown too
      harness.player.emit(
        (playback) => playback.copyWith(error: 'broken frame'),
      );
      await _settle();
  ...
  test(
    'перемотка на 10 секунд: нажатия подряд складываются, границы видео не переходятся',
    () async {
      final harness = _Harness();

      await harness.open(
        testLibraryVideo('a', position: const Duration(minutes: 1)),
      );
  ...
  test(
    'позиция на экране обновляется не чаще интервала, остальное — сразу',
    () async {
      final harness = _Harness();
  ...
  test(
    'громкость: стрелки меняют её в пределах 0–100%, M выключает и возвращает звук',
    () async {
      final harness = _Harness();
