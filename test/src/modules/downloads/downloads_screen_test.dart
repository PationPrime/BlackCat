...
import 'package:peeky_cat/src/modules/downloads/module.dart';
...
void main() {
  ...
  testWidgets('разделы: активная загрузка, очередь и завершённые', (
    tester,
  ) async {
    final app = TestApp();
    ...
    app.videoRepository.downloads.single.succeed(
      r'C:\Downloads\a.mp4',
      sizeBytes: 70000000,
    );
    await app.settle(tester);
  ...
  testWidgets('удаление начатой загрузки спрашивает подтверждение', (
    tester,
  ) async {
    final app = TestApp();
    ...
    final remove = find.descendant(
      of: find.byType(ActiveDownloadCard),
      matching: find.byTooltip('Убрать'),
    );
  ...
  testWidgets('«Скачать сейчас» в очереди делает видео активным', (
    tester,
  ) async {
    final app = TestApp();
  ...
  testWidgets(
    '«Ошибка скачивания» появляется после очереди только с упавшими видео',
    (tester) async {
      final app = TestApp();
      ...
      app.videoRepository.downloads.single.failWith(
        const VideoFailure(
          code: 'mux',
          message: 'Не удалось собрать файл: нет места',
        ),
      );
      await app.settle(tester);
