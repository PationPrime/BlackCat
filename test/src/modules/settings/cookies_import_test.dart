...
void main() {
  ...
  testWidgets('карточка cookies: инструкция, ссылка на FAQ, ошибки файла и ссылки', (tester) async {
    final app = TestApp();
    ...
    expect(find.descendant(of: card, matching: find.textContaining('в формате JSON')), findsOneWidget);
  ...
  testWidgets('импорт из обычных настроек остаётся на странице; удаление cookies выходит из аккаунта', (tester) async {
    final app = TestApp();
  ...
  testWidgets('запрос импорта подсвечивает карточку и после импорта возвращает на запросившую страницу', (tester) async {
    final app = TestApp();

    await app.pumpPage(tester, const SettingsScreen());

    app.navigationController.openCookiesImport(returnTab: AppTabModel.downloads);
    await app.settle(tester);
    ...
    expect(app.navigationController.state, const AppNavigationState(tab: AppTabModel.downloads));
    expect(tester.widget<CookiesCard>(find.byType(CookiesCard)).highlighted, isFalse);
