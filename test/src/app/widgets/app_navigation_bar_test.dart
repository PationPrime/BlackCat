...
const _items = [
  ...
  AppNavigationBarItemData(title: 'Загрузки', icon: Icons.download_rounded, badge: 3),
...
final class _Bar {
  ...
  Future<void> pump(WidgetTester tester, {bool compact = false}) => tester.pumpWidget(
    MaterialApp(
...
double _pillTop(WidgetTester tester) => tester
    .widget<Positioned>(
      find.descendant(of: find.byType(AppNavigationBar), matching: find.byType(Positioned)).first,
    )
    .top!;

Color? _titleColor(WidgetTester tester, String title) => tester.widget<Text>(find.text(title)).style?.color;

void main() {
  testWidgets('таблетка выбора плавно переезжает к нажатой кнопке, цвета меняются по пути', (tester) async {
    final bar = _Bar();

    await bar.pump(tester);

    final accentText = AppThemeData.darkTheme.extension<AppThemeColors>()!.onAccent;
  ...
  testWidgets('компактный навбар: только значки, названия в подсказках', (tester) async {
    final bar = _Bar();

    await bar.pump(tester, compact: true);

    expect(tester.getSize(find.byType(AppNavigationBar)).width, AppNavigationBar.compactWidth);
