...
void main() {
  ...
  test(
    'импорт cookies: настройки с запросом, после импорта — страница, которая просила',
    () {
      final controller = AppNavigationController();
      ...
      expect(
        controller.state.cookiesImport,
        const CookiesImportRequest(id: 1, returnTab: AppTabModel.downloads),
      );
  ...
  test(
    'уход из настроек отменяет запрос импорта; без запроса finishCookiesImport ничего не делает',
    () {
      final controller = AppNavigationController();
