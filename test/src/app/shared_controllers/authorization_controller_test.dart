...
void main() {
  test(
    'checkAuthorization: сохранённая сессия подключает аккаунт без окна',
    () async {
  ...
  test(
    'checkAuthorization: импортированный cookies.txt помнит свой файл',
    () async {
      ...
      final controller = AuthorizationController(
        authenticationRepository: FakeAuthenticationRepository(
          restoreResult: (failure: null, data: session),
        ),
      );
      ...
      expect(
        controller.state.session?.cookiesFilePath,
        r'C:\Users\user\cookies.txt',
      );
    },
  );
  ...
  test('signIn: ошибка попадает в состояние', () async {
    ...
    final controller = AuthorizationController(
      authenticationRepository: FakeAuthenticationRepository(
        signInResult: (failure: failure, data: null),
      ),
    );
  ...
  test(
    'importCookies: импорт подключает аккаунт, ошибка не попадает в общее состояние',
    () async {
      final session = AccountSessionModel.cookiesFile(
        cookiesFilePath: r'C:\Users\user\cookies.txt',
      ...
      final repository = FakeAuthenticationRepository(
        importResult: (failure: null, data: session),
      );
      final controller = AuthorizationController(
