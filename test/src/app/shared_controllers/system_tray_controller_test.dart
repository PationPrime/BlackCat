...
DownloadTaskModel _task({int downloadedBytes = 420, DownloadTaskStatus status = DownloadTaskStatus.downloading}) =>
    DownloadTaskModel(
      id: 'a',
      video: const VideoInfoModel(id: 'a', title: 'Обзор', url: 'https://youtu.be/a', qualities: []),
      quality: const QualityModel(id: '1080', kind: QualityKind.video, label: '1080p'),
      status: status,
...
final class _Harness {
  ...
  late final windowController = AppWindowController(appWindowService: windowService);
...
void main() {
  ...
  test('initialize: иконка в трее — закрытие окна прячет приложение туда', () async {
    ...
    expect(harness.trayService.lastMenuLabels, ['Нет активной загрузки', 'Открыть YT Download', 'Свернуть в трей', 'Выйти']);
    expect(harness.trayService.toolTips.last, 'YT Download');
  });
  ...
  test('активная загрузка: название и процент, одинаковые обновления пропускаются', () async {
    final harness = _Harness();

    await harness.start();
    await harness.controller.showActiveDownload(_task());

    expect(harness.trayService.lastMenuLabels.take(2), ['Обзор', 'Скачивание 42%']);
    expect(harness.trayService.toolTips.last, 'YT Download\nОбзор\nСкачивание 42%');
  ...
  test('правый клик: меню со свежим прогрессом, пока открыто — не перестраивается', () async {
    final harness = _Harness();
  ...
  test('пункты меню: открыть, свернуть в трей, выйти с удалением иконки', () async {
    final harness = _Harness();
    ...
    harness.trayService
      ...
      ..emit(const SystemTrayMenuItemClicked(SystemTrayMenuKeys.activeDownload));
    await _settle();

    expect(harness.windowService.calls, containsAllInOrder(['hide', 'show']));

    harness.trayService.emit(const SystemTrayMenuItemClicked(SystemTrayMenuKeys.quit));
    await _settle();
