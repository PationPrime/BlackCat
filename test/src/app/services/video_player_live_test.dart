...
String? _libmpv() => [
  for (final mode in ['Release', 'Profile', 'Debug']) p.join('build', 'windows', 'x64', 'runner', mode, 'libmpv-2.dll'),
].where((path) => File(path).existsSync()).firstOrNull;

/// A test video of [seconds] made by FFmpeg
Future<File> _sample(Directory folder, String name, List<String> codecArguments, {int seconds = 12}) async {
...
void main() {
  ...
  test('система отдаёт длительность и превью видео', () async {
    final video = await _sample(folder, 'clip.mp4', ['-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'aac']);
    final thumbnail = p.join(folder.path, 'thumbnails', 'clip.jpg');
    final metadata = await VideoMetadataServiceImpl().read(video.path, thumbnailPath: thumbnail);
    ...
    final broken = File(p.join(folder.path, 'broken.mp4'))..writeAsBytesSync([1, 2, 3]);
    final nothing = await VideoMetadataServiceImpl().read(broken.path, thumbnailPath: p.join(folder.path, 'none.jpg'));
  ...
  test('libmpv играет H.264, AV1 и VP9: старт с позиции, скорость, перемотка, конец видео', () async {
    ...
    for (final (name, arguments) in [
      ...
      ('vp9.webm', ['-c:v', 'libvpx-vp9', '-deadline', 'realtime', '-c:a', 'libopus']),
    ]) {
      final video = await _sample(folder, name, arguments, seconds: 8);
      final duration = _next(player.stream.duration, (value) => value > Duration.zero);

      await player.open(Media(video.path, start: const Duration(seconds: 3)));

      expect((await duration).inMilliseconds, closeTo(8000, 150), reason: name);
      expect(player.state.position, greaterThanOrEqualTo(const Duration(seconds: 3)), reason: name);
      ...
      await _next(player.stream.position, (value) => value >= const Duration(seconds: 6));
      await _next(player.stream.completed, (value) => value);

      /// The position of the last frame, a little before the duration
      expect(player.state.position, greaterThan(const Duration(seconds: 7)), reason: name);
  ...
  }, skip: _enabled && _libmpv() != null ? false : 'PLAYER_LIVE=1 and a Windows build with libmpv');
}
