...
void main() {
  ...
  setUp(() async {
    ...
    repository = DownloadQueueRepository(
      downloadTaskTableProvider: DownloadTaskTableProvider(
  ...
  Future<void> writePart(
    String taskId,
    DownloadStreamModel stream,
    int length,
  ) async {
    final directory = await fileSystemService.downloadWorkDirectory(taskId);

    await File(
      DownloadPartFiles.path(directory.path, stream),
    ).writeAsBytes(List.filled(length, 7));
  }

  test(
    'restoreTasks берёт скачанные байты из длины недокачанных файлов',
    () async {
  ...
  test(
    'restoreTasks берёт скачанные байты слайсов из state встроенного загрузчика, а не из длины файлов',
    () async {
      await repository.saveTasks([
        _task(
          'a',
      ...
      await File(
        DownloadPartFiles.path(directory.path, _streams.first),
      ).delete();

      expect(
        (await repository.restoreTasks()).requireData.single.downloadedBytes,
        250,
      );
    },
  );
  ...
  test(
    'restoreTasks стирает рабочие папки загрузок, которых нет в очереди, и готовых',
    () async {
      await repository.saveTasks([
      ...
      final workRoot = Directory(
        p.join(root.path, StorageConstants.downloadWorkFolder),
      );
      final left = [
        await for (final entity in workRoot.list()) p.basename(entity.path),
      ];

      expect(left, ['paused']);
      expect(
        result.requireData
            .firstWhere((task) => task.id == 'done')
            .downloadedBytes,
        4000,
      );
    },
  ...
  test(
    'removeTasks удаляет загрузку из базы вместе с недокачанными файлами',
    () async {
      await repository.saveTasks([
        _task('a'),
        _task('b', section: DownloadTaskSection.queue),
      ]);
      ...
      expect(
        await Directory(
          p.join(root.path, StorageConstants.downloadWorkFolder, 'a'),
        ).exists(),
        isFalse,
      );
    },
  ...
  test(
    'saveThumbnail сохраняет копию превью в папку приложения с расширением из ссылки',
    () async {
      final task = _task(
        'a',
        thumbnail: 'https://i.ytimg.com/vi_webp/video-a/maxresdefault.webp?v=1',
      );
      ...
      expect(
        path,
        p.join(root.path, StorageConstants.thumbnailsFolder, 'a.webp'),
      );
  ...
  test(
    'скачанное видео хранит размер, время завершения и путь к превью',
    () async {
  ...
  test(
    'removeTasks удаляет копию превью, restoreTasks — превью загрузок, которых нет',
    () async {
      await repository.saveTasks([
        ...
        _task(
          ...
          thumbnail: 'https://i.ytimg.com/vi/removed/hq.jpg',
        ),
      ]);

      final keptPath = (await repository.saveThumbnail(
        _task('kept', thumbnail: 'https://i.ytimg.com/vi/kept/hq.jpg'),
      )).requireData;
      final removedPath = (await repository.saveThumbnail(
        _task('removed', thumbnail: 'https://i.ytimg.com/vi/removed/hq.jpg'),
      )).requireData;
      final orphanPath = await fileSystemService.thumbnailPath(
        'orphan',
        extension: 'jpg',
      );
