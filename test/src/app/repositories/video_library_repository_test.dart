...
final class _FakeVideoMetadataService implements VideoMetadataService {
  ...
  @override
  Future<VideoFileMetadataModel> read(String videoPath, {String? thumbnailPath}) async {
    ...
    return VideoFileMetadataModel(duration: durations[name], hasThumbnail: hasThumbnail);
  }
}

final class _Harness {
  final Directory root;
  final database = AppDatabase.forTesting(DatabaseConnection(NativeDatabase.memory()));
  final metadata = _FakeVideoMetadataService();

  late final libraryProvider = LibraryVideoTableProvider(databaseInstance: database);
  late final downloadsProvider = DownloadTaskTableProvider(databaseInstance: database);
  late final repository = VideoLibraryRepository(
    ...
    localVideoLibraryDataSource: LocalVideoLibraryDataSourceImpl(fileSystemService: _TempFileSystemService(root.path)),
  ...
  Future<File> addFile(String name, {int size = 100, required DateTime modified}) async {
...
void main() {
  ...
  test('в библиотеку попадают только видео из самой папки, новые сверху; скачанное берёт название, длительность и превью загрузки', () async {
    ...
    await harness.addFile(p.join('nested', 'c.mp4'), modified: DateTime(2026, 9, 3));

    final downloadThumbnail = File(p.join(harness.root.path, 'app', 'Thumbnails', 'task-a.jpg'));
    ...
    expect([for (final video in videos) video.title], ['b', 'Обзор: часть 1']);
  ...
  test('позиция просмотра переживает повторное чтение; пропавший файл удаляет запись и превью', () async {
    final fileA = await harness.addFile('a.mp4', modified: DateTime(2026, 9, 1));
    ...
    await harness.repository.savePosition(
      a.copyWith(position: const Duration(minutes: 3, seconds: 25), duration: const Duration(minutes: 8)),
    );
    ...
    expect([for (final video in remaining) p.basename(video.path)], ['b.mp4']);
    expect([for (final video in await harness.libraryProvider.getVideos()) p.basename(video.path)], ['b.mp4']);
    expect(await File(loaded.thumbnailPath!).exists(), isFalse);
  });

  test('заменённый файл читается заново: позиция сбрасывается, система спрашивается снова', () async {
    ...
    await harness.repository.savePosition(a.copyWith(position: const Duration(minutes: 2)));
  ...
  test('метаданные: система спрашивается только о недостающем, а сохранённая позиция не затирается', () async {
    await harness.addFile('a.mp4', modified: DateTime(2026, 9, 1));
    harness.metadata.durations['a.mp4'] = const Duration(seconds: 42);

    final [a] = await harness.sync();

    await harness.repository.savePosition(a.copyWith(position: const Duration(seconds: 20)));
  ...
  test('пропавшая папка — ошибка, а библиотека остаётся: диск мог быть отключён', () async {
    await harness.addFile('a.mp4', modified: DateTime(2026, 9, 1));
    await harness.sync();
    await Directory(harness.folder).rename(p.join(harness.root.path, 'Moved'));
  ...
  test('в Linux пути с разным регистром — разные видео', () async {
    final linuxRepository = VideoLibraryRepository(
      ...
      localVideoLibraryDataSource: _NamesDataSource(['/videos/Clip.mp4', '/videos/clip.mp4']),
...
final class _NamesDataSource implements LocalVideoLibraryDataSource {
  ...
  Future<List<LocalVideoFileModel>> getVideoFiles(String folder) async => [
    for (final path in paths) LocalVideoFileModel(path: path, sizeBytes: 1, modifiedAt: DateTime(2026)),
  ];
  ...
  @override
  Future<String> thumbnailPath(String videoId, {required String version, required String extension}) async =>
