...
const _ytDlpUrl = 'https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe';
const _ytDlpSumsUrl = 'https://github.com/yt-dlp/yt-dlp/releases/latest/download/SHA2-256SUMS';
const _denoUrl = 'https://github.com/denoland/deno/releases/latest/download/deno-x86_64-pc-windows-msvc.zip';
...
final class _ToolsYtDlpService implements YtDlpService {
  ...
  _ToolsYtDlpService(this.toolsFolder, {this.systemYtDlp, this.systemJsRuntime});
  ...
  Future<DependencyToolModel?> _bundled(String name, String version) async {
    ...
    return DependencyToolModel(name: name, executable: path, version: version, isBundled: true);
  }

  @override
  Future<YtDlpSetupModel> setup({bool refresh = false}) async => YtDlpSetupModel(
...
final class _FakeRemote implements RemoteDependencyDataSource {
  ...
  Future<void> downloadFile(
    ...
    if (downloadGate case final gate?) {
      await Future.any([gate.future, if (cancelToken != null) cancelToken.whenCancel]);
    }

    if (cancelToken?.isCancelled ?? false) {
      await File(path).delete();

      throw DioException.requestCancelled(requestOptions: RequestOptions(path: url), reason: 'stopped');
    }
  ...
  static DioException _notFound(String url) => DioException.badResponse(
    ...
    response: Response(requestOptions: RequestOptions(path: url), statusCode: 404),
  );
}
...
void main() {
  ...
  final denoZip = ZipEncoder().encodeBytes(Archive()..addFile(ArchiveFile.bytes('deno.exe', denoBytes)));
  ...
  setUp(() async {
    ...
    remote = _FakeRemote({
      _ytDlpSumsUrl: utf8.encode('${_sha(ytDlpBytes)}  yt-dlp.exe\n${'0' * 64}  yt-dlp_macos\n'),
  ...
  List<String> leftovers() => Directory(tools).existsSync()
      ? [for (final entity in Directory(tools).listSync()) p.basename(entity.path)]
      : const [];

  test('ставит yt-dlp и Deno из релизов GitHub, сверяя контрольные суммы', () async {
    final progress = <DependencyInstallProgressModel>[];
    final result = await repository(_ToolsYtDlpService(tools)).installMissing(onProgress: progress.add);
    ...
    expect(remote.requests, [_ytDlpSumsUrl, _ytDlpUrl, _denoSumUrl, _denoUrl]);

    expect([for (final step in progress) (step.kind, step.stage)], [
  ...
  test('ставится только недостающее: Node.js уже есть', () async {
    const node = DependencyToolModel(name: 'node', executable: 'node', version: 'v24.12.0');
  ...
  test('всё уже установлено: ничего не скачивается', () async {
    const ytDlp = DependencyToolModel(name: 'yt-dlp', executable: 'python', version: '2026.08.19');
    const node = DependencyToolModel(name: 'node', executable: 'node', version: 'v24.12.0');
    final result = await repository(
      _ToolsYtDlpService(tools, systemYtDlp: ytDlp, systemJsRuntime: node),
    ).installMissing(onProgress: (_) {});

    expect(result.requireData, const YtDlpSetupModel(ytDlp: ytDlp, jsRuntime: node));
    expect(remote.requests, isEmpty);
  });

  test('несовпавшая контрольная сумма останавливает установку и удаляет файл', () async {
    remote.files[_ytDlpUrl] = utf8.encode('tampered');

    final result = await repository(_ToolsYtDlpService(tools)).installMissing(onProgress: (_) {});

    expect(result.failure?.code, const DependencyErrorCodes().checksumMismatch);
    expect(result.failure?.message, contains('yt-dlp'));
    expect(leftovers(), isEmpty);
    expect(remote.requests, isNot(contains(_denoUrl)));
  });

  test('файла нет в списке контрольных сумм — установка не начинается', () async {
    remote.files[_ytDlpSumsUrl] = utf8.encode('${'0' * 64}  yt-dlp_macos\n');

    final result = await repository(_ToolsYtDlpService(tools)).installMissing(onProgress: (_) {});

    expect(result.failure?.code, const DependencyErrorCodes().checksumMissing);
  ...
  test('нет сети: причина называет программу и GitHub', () async {
    ...
    expect(result.failure?.message, 'Не удалось скачать yt-dlp: нет соединения с GitHub.');
  });
  ...
  test('архив без программы и неработающая программа — понятные ошибки', () async {
    remote.files[_denoUrl] = ZipEncoder().encodeBytes(Archive()..addFile(ArchiveFile.string('README.md', 'deno')));
    remote.files[_denoSumUrl] = utf8.encode('${_sha(remote.files[_denoUrl]!)}  deno-x86_64-pc-windows-msvc.zip\n');

    final extractResult = await repository(_ToolsYtDlpService(tools)).installMissing(onProgress: (_) {});

    expect(extractResult.failure?.code, const DependencyErrorCodes().extract);
    expect(leftovers(), ['yt-dlp.exe']);

    remote.files[_denoUrl] = denoZip;
    remote.files[_denoSumUrl] = utf8.encode('${_sha(denoZip)}  deno-x86_64-pc-windows-msvc.zip\n');

    final brokenService = _ToolsYtDlpService(tools)..broken = true;
    final brokenResult = await repository(brokenService).installMissing(onProgress: (_) {});

    expect(brokenResult.failure?.code, const DependencyErrorCodes().notWorking);
  ...
  test('для системы без готовых сборок установка недоступна', () async {
    ...
    expect(result.failure?.code, const DependencyErrorCodes().unsupportedPlatform);
    expect(remote.requests, isEmpty);
  });
}
