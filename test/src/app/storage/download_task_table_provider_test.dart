...
const _streams = [
  DownloadStreamDto(role: DownloadStreamRole.video, itag: 137, contentLength: 50000000),
  DownloadStreamDto(role: DownloadStreamRole.audio, itag: 140, contentLength: 2428800),
];
...
void main() {
  ...
  test('saveTasks и getTasks: загрузка вместе с потоками, по разделам и местам', () async {
    ...
    expect([for (final stream in active.streams) (stream.role, stream.itag, stream.contentLength)], [
  ...
  test('saveTasks перезаписывает загрузку и её потоки', () async {
    ...
    await provider.saveTasks([
      _task('a', section: DownloadTaskSection.active, status: DownloadTaskStatus.downloading, streams: [_streams.last]),
    ]);
  ...
  test('updateProgress пишет только счётчики байт', () async {
    await provider.saveTasks([_task('a', status: DownloadTaskStatus.downloading)]);
    await provider.updateProgress(taskId: 'a', downloadedBytes: 1024, totalBytes: 4096);
  ...
  test('deleteTasks удаляет загрузку и каскадно её потоки', () async {
    await provider.saveTasks([_task('a', streams: _streams), _task('b', position: 1)]);
