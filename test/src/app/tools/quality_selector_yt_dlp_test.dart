...
final _formats = [
  ...
  _format('hls-1080p', ext: 'mp4', vcodec: 'avc1.640028', width: 1920, height: 1080, protocol: 'https'),
...
final _dubbedFormats = [
  _format('140-0', ext: 'm4a', acodec: 'mp4a.40.2', abr: 129.484, filesize: 7743522, languagePreference: -1),
  _format('140-1', ext: 'm4a', acodec: 'mp4a.40.2', abr: 129.491, filesize: 7743244, languagePreference: 10),
  _format('251-0', ext: 'webm', acodec: 'opus', abr: 123, filesize: 7356590, languagePreference: -1),
  _format('251-1', ext: 'webm', acodec: 'opus', abr: 136, filesize: 8157611, languagePreference: 10),
  _format('160', ext: 'mp4', vcodec: 'avc1.4d400c', width: 256, height: 144, fps: 25, filesize: 6609946),
  _format('137', ext: 'mp4', vcodec: 'avc1.640028', width: 1920, height: 1080, fps: 25, filesize: 167914982),
];

void main() {
  test('itagOf: itag из идентификатора yt-dlp, включая дорожки, DRC и апскейл', () {
  ...
  test('muxableRawFormats: только MP4/M4A по HTTPS с точным размером и itag', () {
    ...
    expect(
      [for (final format in QualitySelector.muxableRawFormats(_dubbedFormats)) format['format_id']],
      ['140-0', '140-1', '160', '137'],
    );
  });
  ...
  test('дублированное видео: качества есть, скачивается оригинальная дорожка, её размер и показывается', () {
    final qualities = QualitySelector.buildQualities(
      QualitySelector.muxableRawFormats(_dubbedFormats),
      canMerge: true,
    );

    expect([for (final quality in qualities) quality.id], ['1080', '144', QualityModel.audioId]);
  ...
  test('сжатый звук (DRC) и апскейл берутся, только если нет оригинала', () {
    final variants = [
      _format('140-drc', ext: 'm4a', acodec: 'mp4a.40.2', abr: 140, filesize: 5000),
      _format('140', ext: 'm4a', acodec: 'mp4a.40.2', abr: 129, filesize: 4000),
      _format('137-sr', ext: 'mp4', vcodec: 'avc1.640028', width: 1920, height: 1080, fps: 30, tbr: 3000),
      _format('137', ext: 'mp4', vcodec: 'avc1.640028', width: 1920, height: 1080, fps: 30, tbr: 2500),
    ];
    ...
    expect(QualitySelector.selectRawStreams(onlyVariants, '1080').video?['format_id'], '137-sr');
    expect(QualitySelector.selectRawStreams(onlyVariants, '1080').audio['format_id'], '140-drc');
  });

  test('без звуковой дорожки видео не предлагается: склеивать не с чем', () {
    final videoOnly = [
      _format('137', ext: 'mp4', vcodec: 'avc1.640028', width: 1920, height: 1080, fps: 25),
    ];

    expect(QualitySelector.buildQualities(videoOnly, canMerge: true), isEmpty);
    expect(() => QualitySelector.selectRawStreams(videoOnly, '1080'), throwsArgumentError);
  });
