...
void main() {
  ...
  test('parseProgress: байты, размер, скорость и остаток времени', () {
    final progress = YtDlpOutput.parseProgress(
      '[progress] downloading 14369139 31457280 NA 5536044.34 3',
    );
  ...
  test(
    'parseProgress: примерный размер, когда точного нет, и неизвестные поля',
    () {
      final progress = YtDlpOutput.parseProgress(
        '[progress] finished NA NA 2048.0 NA NA',
      );
  ...
  test('parseProgress пропускает остальной вывод', () {
    expect(
      YtDlpOutput.parseProgress(
        '[download] Destination: C:\\work\\video-137-1',
      ),
      isNull,
    );
    expect(
      YtDlpOutput.parseProgress('[info] probe: Downloading 1 format(s): 140'),
      isNull,
    );
  });
  ...
  test('toException: проверка на бота и возраст просят войти', () {
    ...
    final age = YtDlpOutput.toException(
  ...
  test('toException: известные ошибки без входа', () {
    expect(
      YtDlpOutput.toException(
        'ERROR: [youtube] abc: Private video. Sign in if you have access',
      ).code,
      codes.privateVideo,
    );
    expect(
      YtDlpOutput.toException('ERROR: [youtube] abc: Video unavailable').code,
      codes.videoUnavailable,
    );
    expect(
      YtDlpOutput.toException(
        'ERROR: [youtube] abc: Requested format is not available',
      ).code,
      codes.qualityUnavailable,
    );
  });

  test(
    'toException: неизвестная ошибка — первое предложение последней строки ERROR',
    () {
      ...
      expect(
        ...
        'unable to download video data: HTTP Error 500: Internal.',
      );
