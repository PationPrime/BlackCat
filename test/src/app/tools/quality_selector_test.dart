...
Map<String, dynamic> _adaptive(
  ...
  'mimeType': mime,
  'url':
      'https://rr1.googlevideo.com/videoplayback?itag=$itag&n=CHALLENGE$itag',
  'width': ?width,
  'height': ?height,
  'fps': ?fps,
  'bitrate': bitrate,
    ...
    'adaptiveFormats': [
      ...
      _adaptive(
        ...
        extra: {
          ...
          'audioTrack': {'audioIsDefault': false},
        },
      ),
      _adaptive(
        140,
...
void main() {
  group('buildQualities', () {
    ...
    test('размер — видео плюс звуковая дорожка, которая к нему приклеится', () {
      final byId = {
        for (final quality in QualitySelector.buildQualities(
          _formats,
          canMerge: true,
        ))
          quality.id: quality,
      ...
      expect(
        byId['720']!.size,
        60000000 + 10000000,
        reason: 'H.264 при равном fps',
      );
      expect(
        byId['1080']!.size,
        150000000 + 10000000,
        reason: 'fps важнее кодека',
      );
    ...
    test('без склейки доступны только форматы со звуком', () {
      expect(
        QualitySelector.buildQualities(
          _formats,
          canMerge: false,
        ).map((quality) => [quality.id, quality.size]),
        [
          ['360', 30000000],
          ['audio', 10000000],
        ],
    ...
    test('вертикальное видео подписывается по короткой стороне', () {
      ...
      expect(
        QualitySelector.buildQualities(shorts, canMerge: true).first.label,
        '1080p',
      );
    });
  });
  ...
  group('потоки YouTube', () {
    test('список качеств и выбор потоков по тем же правилам', () {
      expect(
        QualitySelector.buildStreamQualities(
          _streams,
      ...
      expect(
        hd.audio.audioIsDefault,
        isTrue,
        reason: 'оригинальная дорожка дублированного видео',
      );
      expect(QualitySelector.selectStreams(_streams, 'audio').video, isNull);
      expect(
        () => QualitySelector.selectStreams(_streams, '2160'),
        throwsArgumentError,
      );
    });
  });
  ...
  test('pickDefaultQuality: лучшее разрешение не выше 1080p', () {
    QualityModel video(int resolution) => QualityModel(
      id: '$resolution',
    ...
    expect(
      QualitySelector.pickDefaultQuality([
        video(2160),
        video(1080),
        video(720),
        audio,
