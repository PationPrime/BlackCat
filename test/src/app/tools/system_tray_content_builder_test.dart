...
DownloadTaskModel _task({
  ...
  id: 'a',
  video: VideoInfoModel(id: 'a', title: title, url: 'https://youtu.be/a', qualities: const []),
...
void main() {
  ...
  test('без активной загрузки: неактивная подпись и действия с окном', () {
    ...
    expect(_labels(content), ['Нет активной загрузки', 'Открыть YT Download', 'Свернуть в трей', 'Выйти']);
    expect((content.menu.first as SystemTrayMenuActionModel).enabled, isFalse);
    expect(content.toolTip, 'YT Download');
  });
  ...
  test('длинное название сворачивается в троеточие', () {
    final content = SystemTrayContentBuilder.build(_task(title: 'Название ' * 20));

    expect(_labels(content).first, endsWith('…'));
    expect(_labels(content).first.runes.length, SystemTrayText.menuItemMaxLength);
  });

  test('progressOf: пауза, склейка и подготовка', () {
    expect(SystemTrayContentBuilder.progressOf(_task(status: DownloadTaskStatus.paused)), 'На паузе · 42%');
    expect(SystemTrayContentBuilder.progressOf(_task(status: DownloadTaskStatus.processing)), 'Склеиваем видео и звук…');
    expect(SystemTrayContentBuilder.progressOf(_task(downloadedBytes: 0, totalBytes: null)), 'Готовим загрузку…');
  });
}
