...
final class _StreamServer {
  ...
  /// Sizes `HEAD` gives instead of the real ones
  final claimedLengths = <String, int>{};
      ...
      try {
        ...
        if (request.method == 'HEAD') {
          response.contentLength =
              claimedLengths[request.uri.queryParameters['itag']] ??
              content.length;
        ...
        final [start, end] = [
          for (final part in range.split('-')) int.parse(part),
        ];
        final body = content.sublist(start, math.min(end + 1, content.length));

        requestedRanges
            .putIfAbsent(request.uri.queryParameters['itag']!, () => [])
            .add(range);
        response.contentLength = body.length;

        for (var offset = 0; offset < body.length; offset += 256 * 1024) {
          response.add(
            body.sublist(offset, math.min(offset + 256 * 1024, body.length)),
          );
...
void main() {
  ...
  final video = Uint8List.fromList([
    for (var i = 0; i < 25 * _mib + 123; i++) i % 251,
  ]);
  final audio = Uint8List.fromList([
    for (var i = 0; i < 3 * _mib + 5; i++) i % 13,
  ]);
  ...
  test(
    'видео и звук качаются слайсами по 10 МиБ параллельно, state в конце удаляется',
    () async {
  ...
  test(
    'файл, записанный по порядку (yt-dlp или прежней версией), продолжается с его длины',
    () async {
      const alreadyDownloaded = 12 * _mib + 7;

      await File(
        targets().first.path,
      ).writeAsBytes(video.sublist(0, alreadyDownloaded));

      await download();

      expect(
        server.requestedRanges['137'],
        unorderedEquals(['$alreadyDownloaded-20971519', '20971520-26214522']),
      );
      expect(await File(targets().first.path).readAsBytes(), video);
    },
  );
  ...
  test(
    'остановка сохраняет слайсы в state, продолжение докачивает только недостающее',
    () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<VideoException>().having(
            (error) => error.code,
            'code',
            const VideoErrorCodes().canceled,
          ),
        ),
      );
      ...
      final savedVideo = saved.fileByIdentity(
        'video-137-${video.length}.part',
      )!;
      ...
      expect(
        ...
        unorderedEquals([
          for (var slice = 0; slice < 3; slice++)
            if (savedVideo.sliceCounters[slice] <
                math.min(10 * _mib, video.length - slice * 10 * _mib))
  ...
  test(
    '403 значит, что ссылки устарели: прогресс остаётся для новых ссылок',
    () async {
      server.forbidden = true;

      await expectLater(
        download(),
        throwsA(isA<MediaStreamLinksExpiredException>()),
      );
  ...
  test(
    'места на диске не хватает — ошибка с размерами, поток не качается',
    () async {
      final free = const SystemDiskSpace().availableBytes(root.path)!;
      final length = free + (1 << 30);

      server.claimedLengths['137'] = length;
      ...
      await expectLater(
        dataSource.downloadStreams(
          ...
          streams: [
            MediaStreamTarget(
              url: server.url('137'),
              path: partPath('video-137-$length.part'),
              length: length,
              fingerprint: 'video-137-$length.part',
            ),
          ],
        ),
        throwsA(
          isA<VideoException>()
              .having((e) => e.code, 'code', const VideoErrorCodes().diskFull)
              .having(
                (e) => e.args.keys,
                'args',
                unorderedEquals(['needed', 'available']),
              ),
        ),
      );
      expect(server.requestedRanges, isEmpty);
      expect(File(partPath('video-137-$length.part')).existsSync(), isFalse);
    },
  );
}
