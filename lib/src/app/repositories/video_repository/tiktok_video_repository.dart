...
/// A video with its qualities, while its links and cookies are fresh
final class _ResolvedTikTokVideo {
  final TikTokVideoDto video;
  final DateTime resolvedAt;

  _ResolvedTikTokVideo(this.video) : resolvedAt = DateTime.now();

  /// The file links live for hours; a fresh page is taken well before
  bool get expired =>
      DateTime.now().difference(resolvedAt) > const Duration(minutes: 30);
}

/// TikTok videos straight over HTTP (dio), without yt-dlp.
///
/// 1. The video page carries the video data and sets the cookies the file
///    links need.
/// 2. The quality picks one of the MP4 files of the page: video and audio
///    come together, nothing is muxed.
...
///    continues
final class TikTokVideoRepository implements VideoRepositoryInterface {
  final RemoteTikTokDataSource _remoteTikTokDataSource;
  final FileSystemService _fileSystemService;

  /// By the link the user gave and by the video id
  final _resolvedVideos = <String, _ResolvedTikTokVideo>{};

  TikTokVideoRepository({
    required this._remoteTikTokDataSource,
  ...
  Future<OperationResult<VideoInfoModel>> getVideoInfo(String url) async {
    try {
      ...
      final qualities = TikTokQualities.build(video.formats);
  ...
  Future<OperationResult<DownloadedFileModel>> downloadVideo({
    ...
    try {
      ...
      Future<String> download(_ResolvedTikTokVideo resolved) => _download(
        resolved.video,
        taskId: taskId,
        quality: quality,
  ...
  Future<_ResolvedTikTokVideo> _resolve(TikTokVideoLink link) async {
    final resolved = _ResolvedTikTokVideo(
      await _remoteTikTokDataSource.getVideo(link),
    );

    _resolvedVideos[_keyOf(link)] = resolved;
    _resolvedVideos[resolved.video.id] = resolved;

    return resolved;
  }

  Future<String> _download(
    TikTokVideoDto video, {
    required String taskId,
    required String quality,
    ...
    final path = p.join(
      workDirectory,
      '${TikTokConstants.filePrefix}${format.bitrate}.mp4',
    );

    await _deleteStaleFiles(workDirectory, keep: p.basename(path));

    await _remoteTikTokDataSource.downloadVideo(
      downloadId: taskId,
      video: video,
      format: format,
      path: path,
  ...
  Future<void> _deleteStaleFiles(
    ...
    await for (final entity in Directory(workDirectory).list()) {
      ...
      if (entity is File &&
          name.startsWith(TikTokConstants.filePrefix) &&
          name != keep) {
