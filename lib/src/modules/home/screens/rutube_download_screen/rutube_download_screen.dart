...
/// RuTube videos and shorts by link. RuTube needs no account: the videos
/// it shows are downloaded as they are
@RoutePage()
class RuTubeDownloadScreen extends StatelessWidget implements AutoRouteWrapper {
  const RuTubeDownloadScreen({super.key});
  ...
  Widget wrappedRoute(BuildContext context) => BlocProvider<AddVideoController>(
    create: (context) => AddVideoController(
      ...
      source: VideoSourceModel.rutube,
    ),
    child: this,
  );
  ...
  Widget build(BuildContext context) => VideoSearchView(
    title: LocaleKeys.app_home_rutube_title.tr(),
    subtitle: LocaleKeys.app_home_rutube_subtitle.tr(),
    urlHint: LocaleKeys.app_home_rutube_url_hint.tr(),
  );
}
